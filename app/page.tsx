...
const getStatus = (dark: boolean): Record<string, StatusConfig> => ({
...
const IcBk = ({ f }: { f?: boolean }) => <svg width="16" height="16" viewBox="0 0 24 24" fill={f ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="m19 21-7-4-7 4V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v16z"/></svg>;
...
interface Scholarship {
  ...
  elig: string[];
  docs: string[];
  tl: { opens: string; closes: string; results: string };
  contact: { web: string; email: string };
  tip: string;
}
...
function Toggle({ on, fn, t }: { on: boolean; fn: () => void; t: Theme }) {
  return (
    <div
      onClick={fn}
...
function Sheet({ t, visible, onClose, title, children }: { t: Theme; visible: boolean; onClose: () => void; title?: string; children: React.ReactNode }) {
...
function Card({ s, t, dk, saved, onSave, appStatus, onClick, editMode, onDelete, idx }: { s: Scholarship; t: Theme; dk: boolean; saved: boolean; onSave: (id: number) => void; appStatus?: string; onClick?: (s: Scholarship) => void; editMode?: boolean; onDelete?: () => void; idx?: number }) {
  const i = idx || 0;
  ...
  const handleMouseUp = () => { if (timer.current) clearTimeout(timer.current); };
...
function NotifPanel({ t, onClose, onOpenDetail }: { t: Theme; onClose: () => void; onOpenDetail: (s: Scholarship) => void }) {
  return (
...
function Detail({ s, t, dk, saved, onSave, apps, onApply, onUpdate, onBack, onAI }: { s: Scholarship; t: Theme; dk: boolean; saved: boolean; onSave: (id: number) => void; apps: Record<number, AppRecord>; onApply: (id: number) => void; onUpdate: (id: number, status: string) => void; onBack: () => void; onAI: () => void }) {
  const [tab, setTab] = useState("about");
  const [showStatus, setShowStatus] = useState(false);
  const appRec = apps[s.id];
  const sc = getStatus(dk);
  const TABS = ["about","eligibility","documents","timeline"];
  const TL: Record<string, string> = { about:"About", eligibility:"Eligibility", documents:"Documents", timeline:"Timeline" };
...
function Onboarding({ t, onDone }: { t: Theme; onDone: (answers: Record<string, string | string[]>) => void }) {
...
function Home({ t, dk, saved, onSave, nav, onOpenDetail, onShowNotifs, apps }: { t: Theme; dk: boolean; saved: number[]; onSave: (id: number) => void; nav: (screen: string) => void; onOpenDetail: (s: Scholarship) => void; onShowNotifs: () => void; apps: Record<number, AppRecord> }) {
...
function Search({ t, dk, saved, onSave, onOpenDetail }: { t: Theme; dk: boolean; saved: number[]; onSave: (id: number) => void; onOpenDetail: (s: Scholarship) => void }) {
  ...
  const toggleF = (f: string) => setFilters(p => p.includes(f) ? p.filter(x => x !== f) : [...p, f]);
...
function SavedWS({ t, dk, savedIds, onSave, apps, onUpdate, onRemove, onOpenDetail }: { t: Theme; dk: boolean; savedIds: number[]; onSave: (id: number) => void; apps: Record<number, AppRecord>; onUpdate: (id: number, status: string) => void; onRemove: (id: number) => void; onOpenDetail: (s: Scholarship) => void }) {
  ...
  const getList = (k: string) => k === "saved" ? SD.filter(s => savedIds.includes(s.id)) : SD.filter(s => apps[s.id]?.status === k);
  const counts = TABS.reduce((a, tb) => ({ ...a, [tb.k]: getList(tb.k).length }), {} as Record<string, number>);
      ...
      <div className="dmax" style={{ margin:"0 auto" }}>
        ...
        {list.length === 0 ? (
          ...
          <div className="grid">
            {list.map((s, i) => (
              <div key={s.id}>
                <Card s={s} t={t} dk={dk} saved={savedIds.includes(s.id)} onSave={onSave} appStatus={apps[s.id]?.status} onClick={!editMode ? onOpenDetail : undefined} editMode={editMode} onDelete={() => { if (tab === "saved") { onSave(s.id); } else { onRemove(s.id); } }} idx={i} />
                {tab !== "saved" && !editMode && (
        ...
        <Sheet t={t} visible={!!statusTarget} onClose={() => setStatusTarget(null)} title="Update Status">
          {Object.entries(sc).map(([k, v]) => (
            <button key={k} className="tap" onClick={() => { if (statusTarget) onUpdate(statusTarget, k); setStatusTarget(null); }} style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 16px", borderRadius:14, border:`2px solid ${statusTarget && apps[statusTarget]?.status === k ? t.ac : t.bd}`, background: statusTarget && apps[statusTarget]?.status === k ? t.as : t.cd, marginBottom:8, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                <span style={{ width:10, height:10, borderRadius:"50%", background:v.c, display:"inline-block" }} />
                <span style={{ fontSize:15, fontWeight:600, color:t.t1 }}>{v.l}</span>
              </div>
              {statusTarget && apps[statusTarget]?.status === k && <span style={{ fontSize:14, color:t.ac }}>✓</span>}
            </button>
          ))}
        </Sheet>
      </div>
...
function AIScreen({ t, onBack }: { t: Theme; onBack: () => void }) {
  ...
  const send = async () => {
    ...
    setTimeout(() => {
      setMsgs(p => [...p, { role:"assistant", text:"I'd be happy to help you with that! Based on your question, I recommend exploring the scholarships in our database that match your profile. Would you like me to suggest some specific options?" }]);
      setLoading(false);
    }, 1500);
  };
...
function ProfileScreen({ t, dk, setDark, profile, setProfile, onReset, apps, savedCount }: { t: Theme; dk: boolean; setDark: React.Dispatch<React.SetStateAction<boolean>>; profile: Record<string, string | string[]> | null; setProfile: React.Dispatch<React.SetStateAction<Record<string, string | string[]> | null>>; onReset: () => void; apps: Record<number, AppRecord>; savedCount: number }) {
  ...
  const openEdit = (k: string, v: string | string[] | undefined) => { setEditField(k); setEditVal(Array.isArray(v) ? v.join(", ") : v || ""); };
            ...
            <div key={f.k} className="tap" onClick={() => openEdit(f.k, profile?.[f.k])} style={{ display:"flex", alignItems:"center", gap:14, padding:"15px 18px", borderBottom: i < FIELDS.length - 1 ? `1px solid ${t.bd}` : "none", cursor:"pointer" }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:15, fontWeight:600, color:t.t1 }}>
                  {profile?.[f.k] ? (Array.isArray(profile[f.k]) ? (profile[f.k] as string[]).slice(0, 2).join(", ") : profile[f.k] as string) : <span style={{ color:t.t3, fontWeight:400 }}>Not set</span>}
                </div>
              </div>
        ...
        <div style={{ background:t.cd, borderRadius:24, overflow:"hidden", boxShadow:`0 2px 14px ${t.sh}`, marginBottom:14 }}>
          ...
          {[
            ...
            <div key={item.k} style={{ display:"flex", alignItems:"center", gap:14, padding:"14px 18px", borderBottom: i < arr.length - 1 ? `1px solid ${t.bd}` : "none" }}>
              ...
              <Toggle on={notifs[item.k as keyof typeof notifs]} fn={() => setNotifs(p => ({ ...p, [item.k]: !p[item.k as keyof typeof notifs] }))} t={t} />
            </div>
          ))}
        </div>
...
function Sidebar({ t, screen, nav }: { t: Theme; screen: string; nav: (screen: string) => void }) {
  const items = [
...
function BottomNav({ t, screen, nav }: { t: Theme; screen: string; nav: (screen: string) => void }) {
  const items = [
...
export default function Nexora() {
  ...
  const [apps, setApps] = useState<Record<number, AppRecord>>({});
  const [profile, setProfile] = useState<Record<string, string | string[]> | null>(null);
  ...
  const applyTo = (id: number) => setApps(p => ({ ...p, [id]: { ...(p[id] || {}), status:"applied", date: new Date().toLocaleDateString() } }));
  const updateApp = (id: number, status: string) => setApps(p => ({ ...p, [id]: { ...p[id], status } }));
  const removeApp = (id: number) => { const np = { ...apps }; delete np[id]; setApps(np); };
  ...
  return (
    <div style={{ background:t.bg, minHeight:"100vh", color:t.t1, fontFamily:"'DM Sans',sans-serif", transition:"background .3s,color .3s" }}>
      ...
      {detail && (
        <Detail s={detail} t={t} dk={dk} saved={saved.includes(detail.id)} onSave={toggleSave} apps={apps} onApply={applyTo} onUpdate={updateApp} onBack={() => setDetail(null)} onAI={() => { setDetail(null); nav("ai"); }} />
      )}
